...
console.log('✅ Hello from GitHub Actions at ' + new Date().toISOString());
		...
		if (unMarkedGames.rows.length > 0) {
			...
			const gamesToMark = await Promise.all(
				completedUnmarkedGames.map(async (game) => {
					...
					const gameAlreadyMarked = await client.query(
						`SELECT * FROM "Pick"
            WHERE game_id = $1
            AND winner IS NOT NULL
            AND marked = TRUE
            AND week = $2`,
						[game.game_id, game.week]
					);

					if (gameAlreadyMarked.rows.length > 0) {
						const markedGame = gameAlreadyMarked.rows[0];

						return {
							...game,
							home_team_score: markedGame.home_team_score,
							away_team_score: markedGame.away_team_score
						};
					}
					...
					let scoreGame = completedGames.find((score) => score.id === game.game_id);

					if (!scoreGame) return game;

					const ht_score = scoreGame.scores?.find(
						(sc) => fullNameToMascot[sc.name] === game.home_team
					)?.score;
					const at_score = scoreGame.scores?.find(
						(sc) => fullNameToMascot[sc.name] === game.away_team
					)?.score;

					home_team_score = ht_score ? parseInt(ht_score) : null;
					away_team_score = at_score ? parseInt(at_score) : null;
