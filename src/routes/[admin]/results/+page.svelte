<script lang="ts">
	...
	let { pmtWeeklyPicks } = $derived(data);
...
>
	...
	<form method="POST" action="?/addLinks" use:enhance class="flex flex-col gap-y-6 justify-center">
		{#if pmtWeeklyPicks && pmtWeeklyPicks?.length > 0}
			{#each pmtWeeklyPicks as team}
				<div class="flex flex-col gap-y-2 justify-center-center">
					<h1 class="text-2xl font-semibold">
						{mascotToFullName[team.homeTeam]} vs {mascotToFullName[team.awayTeam]}
					</h1>
			...
			<button
				class="w-fit px-4 py-1.5 rounded-md bg-lightRed hover:bg-lightRedHover dark:bg-darkRed dark:hover:bg-darkRedHover text-white transition-all duration-300 ease-in-out"
				type="submit"
			>
				Submit
			</button>
